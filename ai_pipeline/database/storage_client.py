...
Usage in ECS Task (entrypoint.py):
    client = StorageClientFactory.from_env()
    client.download_file("uploads/video.mp4", "/tmp/video.mp4")
    client.upload_file("/tmp/proxy.mp4", "proxies/video_proxy.mp4")
    client.upload_bytes(jpeg_bytes, "thumbnails/scene_0001.jpg")
    client.delete_file("proxies/old_proxy.mp4")
"""
...
class StorageClientFactory:
    ...
    def from_env() -> StorageClient:
        ...
        raise ValueError(
            f"Unknown STORAGE_BACKEND='{backend}'. Expected 'minio' or 's3'."
        )
